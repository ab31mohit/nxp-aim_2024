...
class LineFollower(Node):
	...
	def edge_vectors_callback(self, message):
		...
		if not self.ramp_process:
			self.rover_move_manual_mode(SPEED_MAX, self.curr_turn)
			self.curr_speed = SPEED_MAX
	...
	def lidar_callback(self, message):
		...
		if (not self.ramp_started) and  (self.ramp_finished) and (distance < self.ramp_threshold_distance):
			...
			self.ramp_process = True
			# self.rover_move_manual_mode(SPEED_50_PERCENT, self.curr_turn)

		elif self.ramp_started and (not self.ramp_finished) and (self.ramp_slant_height is None):
			self.ramp_upLength.append(distance)
			# inf condition
			if distance > 10:
				self.ramp_slant_height = self.ramp_upLength.popleft()
				print(f"Climbing started, speed={SPEED_50_PERCENT}\n")
				self.curr_speed = SPEED_50_PERCENT
				self.rover_move_manual_mode(SPEED_50_PERCENT, self.curr_turn)
				total_duration = (2.0*self.ramp_slant_height)/SPEED_50_PERCENT
		...
		# elif self.ramp_started and (not self.ramp_finished) and (self.ramp_slant_height is not None):
		# 	print(f"Climbing started, speed={SPEED_50_PERCENT}\n")
		# 	self.rover_move_manual_mode(SPEED_50_PERCENT, self.curr_turn)
		# 	total_duration = (2.0*self.ramp_slant_height)/SPEED_50_PERCENT
		# 	time.sleep(total_duration)
